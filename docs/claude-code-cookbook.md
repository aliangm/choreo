# 用 choreo 驯服 Claude Code

一本 cookbook：列举 Claude Code 常见的"不听话"场景、直接用 prompt 讲为什么不够用、以及用 `choreo` 插件把规矩写成状态机的做法。
...
`choreo` 把这些规矩写成 `workflow.json` 状态机：每个阶段有输入依赖、产出 artifact、pass/fail 判据。偏离了状态机会把它拉回来；到了 `max_epoch` 强制 `escalated`，不会无限打转。
...
/choreo:create "<自然语言描述>"

# 用创建好的 workflow 跑一个任务
/choreo:start --flow=cloud://<你>/<名字> "<任务>"
```

创建完的 workflow 默认发到 hub（<https://choreo.worldstatelabs.com/hub>），跨 session、跨机器都能复用。浏览器里能看 stage timeline + 实时 diff + artifact。
...
发布后跑：
```
/choreo:start --flow=cloud://you/research-first "给 API 加限流"
```
...
**写成 workflow**：
```
/choreo:create "execute → build → test → review 四段式。
- execute：写代码。
...
```
/choreo:create "... 每个 stage 的 instructions 里都显式列：
...
- **改已有 workflow**：`/choreo:create --flow=cloud://you/name "在 qa 后加一个 deploy-dry-run 阶段"` —— 同一个命令既是创建也是编辑。
- **逛别人的 workflow**：<https://choreo.worldstatelabs.com/hub>。
- **中途介入**：`/choreo:interrupt` 暂停，`/choreo:continue` 恢复（支持跨机器，需要 git push 同步代码）。
- **本地模式**：加 `--mode=local` 全离线，state 存在 `<project>/.choreo/`。
