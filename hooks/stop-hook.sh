...
# Session-keyed: every session has its own .choreo/<session_id>/ dir.
...
if is_terminal_status "$STATUS"; then
  case "$STATUS" in
    interrupted)
      ...
      # but handle it gracefully — allow exit, keep state for /choreo:continue
      exit 0
      ;;
    *)
...
# Paused by user — allow exit but KEEP state file for /choreo:continue
...
# choreo:choreo has NEVER been invoked yet. We detect this by
...
# Skill("choreo:choreo") invocation — the stage hasn't actually
...
# choreo:choreo RIGHT NOW, instead of the normal interruptible
...
if [[ -z "$BOOTSTRAP_COMPLETED_AT" ]]; then
  ...
  # without ever invoking `Skill("choreo:choreo")`. The workflow
  ...
  BOOT_REASON="⚙️ Dev workflow: skill driver has not engaged stage \"$STATUS\" (epoch $EPOCH). state.md is missing \`bootstrap_completed_at\` — the field loop-tick.sh writes on its first successful run. Invoke \`Skill(\"choreo:choreo\")\` and run its Step 1 Bash through your Bash tool. Loading the skill or describing the stage in prose does not advance the lifecycle; only running the script does."
...
[[ -n "$SYNC_WARNINGS" ]] && SYSTEM_MSG="${SYSTEM_MSG}  |  sync warnings: ${SYNC_WARNINGS}"
    ...
    SYSTEM_MSG="📋 Dev workflow: $STATUS stage (epoch $EPOCH) — interruptible. Stage instructions: $INSTR. Continue the conversation to proceed, or use /choreo:cancel to abort."
  fi
...
if [[ -f "$ARTIFACT" ]] && [[ "$ARTIFACT_EPOCH" == "$EPOCH" ]] && [[ -n "$ARTIFACT_RESULT" ]]; then
  ...
  if [[ -z "$NEXT" ]]; then
    CONTINUE_PROMPT="[choreo] BLOCKED EXIT — unknown result in artifact.
...
DO NOT STOP."
    ...
    CONTINUE_PROMPT="[choreo] BLOCKED EXIT — stage '$STATUS' DONE (result: $ARTIFACT_RESULT), transition not yet called.
...
DO NOT STOP. The loop is infinite — only /choreo:interrupt or /choreo:cancel stops it."
  fi
else
  # ──────────────────────────────────────────────────────────────
...
${DETAIL_LINES}
Main agent will resume automatically when each completes (no action needed). Use /choreo:interrupt to pause or /choreo:cancel to abort."
...
sync warnings: ${SYNC_WARNINGS}"
  ...
  CONTINUE_PROMPT="[choreo] BLOCKED EXIT — workflow in progress (phase: $STATUS, epoch: $EPOCH).
...
SYSTEM_MSG="🔄 Dev workflow | Phase: $STATUS (epoch $EPOCH) | EXIT BLOCKED — /choreo:interrupt to pause, /choreo:cancel to stop"
[[ -n "$SYNC_WARNINGS" ]] && SYSTEM_MSG="${SYSTEM_MSG}  |  sync warnings: ${SYNC_WARNINGS}"
