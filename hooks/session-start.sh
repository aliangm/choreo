...
#      and by harness PPID. Used as the .choreo/<session_id>/
...
#      env. Writing it to ~/.config/choreo/plugin-root lets SKILL.md and
#      ad-hoc scripts read it back with a single `cat` without a
#      filesystem discovery pattern.
#
# Layout (XDG-split):
#   ~/.config/choreo/plugin-root            ← one-line absolute path
#   ~/.cache/choreo/session-cache/cwd-<sha1-of-pwd>  ← session_id, primary key
#   ~/.cache/choreo/session-cache/ppid-<PPID>        ← session_id, secondary key
...
CACHE_DIR="${HOME}/.cache/choreo/session-cache"
mkdir -p "$CACHE_DIR"
...
if [[ -n "${CLAUDE_PLUGIN_ROOT:-}" ]] && [[ -d "$CLAUDE_PLUGIN_ROOT/scripts" ]]; then
  mkdir -p "${HOME}/.config/choreo"
  echo "$CLAUDE_PLUGIN_ROOT" > "${HOME}/.config/choreo/plugin-root"
fi
