...
#
# Choreo's skill files tell the agent to invoke plugin scripts via a
...
#
# Opt-out: set `CHOREO_NO_PATH_REWRITE=1` in the environment to
...
# Opt-out kill switch.
[[ "${CHOREO_NO_PATH_REWRITE:-}" == "1" ]] && exit 0
...
#
# Line 1: `P=$(cat ~/.config/choreo/plugin-root ...)`
# Line 2: `[[ -n $P && -d $P/scripts ]] || P=$(ls -d ~/.claude/plugins/cache/.../choreo/.../ ...)`
STRIPPED="$(printf '%s' "$CMD" | sed -E '
  /^[[:space:]]*P="?\$\(cat ~\/\.config\/choreo\/plugin-root/d
