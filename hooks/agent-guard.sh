...
# When a choreo is active and Claude launches an Agent, this hook
...
#
# Filter: only deny when the launch target IS the choreo workflow
...
if [[ "$INCOMING_SUBAGENT_TYPE" == "choreo:workflow-subagent" ]]; then
  ...
  if [[ -d "$LEDGER_DIR" ]]; then
    ...
    if [[ -n "$EXISTING_AGENT" ]]; then
      jq -n \
        --arg reason "[choreo] Refusing to launch a second workflow subagent for stage '$STATUS' (epoch $EPOCH) — one is already in flight (agent_id: $EXISTING_AGENT). Wait for it to complete; do not stop, the completion notification will arrive. To abort, run /choreo:interrupt or /choreo:cancel." \
        '{
          "decision": "block",
          "reason": $reason
        }'
...
if [[ "$EXEC_TYPE" == "inline" ]]; then
  ...
  #
  #   a) Main agent is launching `choreo:workflow-subagent` —
  ...
  if [[ "$INCOMING_SUBAGENT_TYPE" == "choreo:workflow-subagent" ]]; then
    cat <<EOF
[choreo] Active workflow (phase: $STATUS, epoch: $EPOCH).
This stage is INLINE — the main agent runs it directly.
Do NOT launch \`choreo:workflow-subagent\` here. If you really meant to advance, transition out of $STATUS first via ${CLAUDE_PLUGIN_ROOT}/scripts/update-status.sh.
...
cat <<EOF
[choreo] Phase: $STATUS (epoch $EPOCH)
