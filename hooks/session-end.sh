...
# active choreo, flip it to `status: interrupted` so another
# Claude session can safely pick it up via /choreo:continue.
#
# The goal is to make hand-off painless: the user doesn't need to
# remember to run /choreo:interrupt before leaving. ESC alone
