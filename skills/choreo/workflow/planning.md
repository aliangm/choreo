...
- **Cloud:** `<CHOREO_SERVER>/s/<session-id>` — defaults to `https://choreo.worldstatelabs.com/s/<session-id>`. Read `<session-id>` from the `session_id:` field in `state.md`; respect `$CHOREO_SERVER` if it's set.
