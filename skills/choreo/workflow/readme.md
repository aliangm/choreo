...
The generic `choreo:workflow-subagent` is launched with `reviewing.md`. Adversarial code review: diffs HEAD against the baseline commit, checks correctness, completeness, design, edge cases, security, **and the Quick Tests result from the execution report**. Code-level issues only — runtime/UX bugs are QA's job.
...
The generic `choreo:workflow-subagent` is launched with `qa-ing.md`. Runs Playwright user-journey tests, maintains a persistent journey-test state file across iterations, distinguishes test bugs (auto-fixed) from app bugs (block progress).
...
- `cancelled` — user ran `/choreo:cancel`
...
```sh
/choreo:create --flow=cloud://your-author/your-name "<describe your changes>"
```
