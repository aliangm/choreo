...
**Cloud mode is the default.** When the user runs `/choreo:start <task>` without any flag, state + artifacts live on the remote **workflowUI** server. The project's `.choreo/` gets nothing.

**To opt out** (fully-offline local mode):
- Pass `--mode=local` to `setup-workflow.sh`, OR
- Export `CHOREO_DEFAULT_MODE=local` in the shell env before launching Claude Code.

**Login**: run `/choreo:login` for authenticated ownership (required to publish cloud workflows). Anonymous sessions are accepted for everything else. Export `CHOREO_SERVER` to point at an alternative deployment.
...
**Cross-machine continuation**: pass `--session <id>` to `/choreo:continue` to resume a cloud session started on another machine. The script rebuilds the local shadow automatically.
...
- For any stage whose `workflow.json` → `stages.<stage>.execution.type` is `"subagent"`, you launch the single generic `choreo:workflow-subagent`. The `agent-guard.sh` PreToolUse hook prints the correct subagent_type / model / mode; pass `prompt: "Execute the current workflow stage."` — the subagent self-resolves its context from state.md + workflow.json.
</CRITICAL>
...
To bridge the gap, `hooks/session-start.sh` writes the absolute plugin root path to **`~/.config/choreo/plugin-root`** on every session start (the hook runs with `$CLAUDE_PLUGIN_ROOT` set). Every Bash-tool call that needs to run a plugin script should read that file:
...
if ! "$P/scripts/loop-tick.sh" >/dev/null 2>&1; then
  ...
  echo "   choreo:choreo drives an existing workflow's stage loop. Routes that bootstrap a workflow:" >&2
  echo "     /choreo:start <task>           — start a fresh workflow" >&2
  echo "     /choreo:continue [--session X] — resume an interrupted/cloud workflow" >&2
  echo "     /choreo:create <desc> — author a new workflow definition" >&2
  exit 1
fi
```
...
Loop:
  ...
  c. Run the stage per its execution type (from TICK):
     ...
     - `"subagent"`:
         ...
         Call the Agent tool:
             - subagent_type: "choreo:workflow-subagent"
...
- **Loop termination**: the workflow stops only when `status` reaches a value listed in `workflow.json` → `terminal_stages` (arrived at via a legitimate transition in the transition table), or the user runs `/choreo:interrupt` or `/choreo:cancel`.
...
- **The loop is infinite** — it stops only on reaching a terminal status, `/choreo:interrupt`, or `/choreo:cancel`.
  - `/choreo:interrupt` — pause and preserve state (resumable via `/choreo:continue`)
  - `/choreo:cancel` — cancel and clear all state
