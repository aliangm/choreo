---
name: create
description: "Create a new workflow suite from a natural-language description, or edit an existing one when --flow=<path> is passed. Dispatches the create-workflow choreo (plan → write → validate loop) — does not write files directly."
---

# Create / Edit Workflow

This skill **dispatches a choreo** that creates or edits a choreo definition. It does NOT write workflow files itself — the choreo's state machine (`planning → writing → validating`) does that, with validator-driven retry until `✓ Workflow validated` prints.

- **Create mode** (no `--flow` flag): the choreo's `planning` stage interviews the user from scratch.
...
Both modes dispatch the same choreo at `$P/skills/create-workflow/workflow`. The difference is a single env var (`CREATE_WORKFLOW_CONTEXT`) passed at dispatch time.
...
```bash
P=$(cat ~/.config/choreo/plugin-root 2>/dev/null)
[[ -n $P && -d $P/scripts ]] || P=$(ls -d ~/.claude/plugins/cache/*/choreo/*/ 2>/dev/null | head -1)
```
...
`NOT_LOGGED_IN` → hard stop: tell the user to run `/choreo:login` first. Do not dispatch.
...
- `AUTHORIZED` → download to a local working dir:
  ...
  ```bash
  SOURCE_DIR="${HOME}/.config/choreo/workflows/${_WF_NAME}"
  mkdir -p "$SOURCE_DIR"
...
The choreo has a `setup_context` run_file that captures this env var. It's how the `planning` stage gets (a) whether this is create or edit, (b) the user's original description, and (c) the source dir for edit mode.
...
`publish_intent` mirrors `$MODE` and tells the choreo's `publishing` stage whether to push to the hub (`cloud`) or skip (`local`).
...
Just a short kebab-case label for THIS choreo run's session (NOT the generated workflow's suffix — planning chooses that). Derive something from `$DESCRIPTION` (first few words kebabed), e.g. `create-lint-wf`, `edit-python-lib`.

### Step 4 — Dispatch the choreo

Branch on `$MODE` to pick BOTH the right workflow source AND the session-mode for `setup-workflow.sh`:

- **`$MODE=cloud`(default)** — use the hub-published anonymous mirror so this choreo session is cloud-tracked (gives the user a live `https://choreo.worldstatelabs.com/s/<sid>` link):
  ```bash
  P=$(cat ~/.config/choreo/plugin-root 2>/dev/null)
  [[ -n $P && -d $P/scripts ]] || P=$(ls -d ~/.claude/plugins/cache/*/choreo/*/ 2>/dev/null | head -1)
...
- **Exit 2 — session already has an active (or interrupted) workflow.** Phase 0 detected a non-terminal `state.md` for this session. The script's stderr lists the existing topic + status. **Do NOT auto-`--force`** — that would silently archive in-progress work. Relay the script's message to the user and offer three choices, then halt:
  - `/choreo:interrupt` — pause the existing workflow (safe, preserves state)
  - `/choreo:continue` — resume the existing workflow (treats this `/create-workflow` call as the one to discard)
  - `/choreo:cancel` — archive (or `--hard` wipe) the existing run, then retry `/create-workflow` with the same args
...
the `commands/create-workflow.md` wrapper will invoke `choreo:choreo`
as its next step.
...
- **DO NOT** add a "Status: create-workflow choreo dispatched" line,
...
- **Live progress**: <choreo.worldstatelabs.com/s/<session_id>>
- **Template (final artifact)**: cloud://<author>/<suffix>
  → visible at <choreo.worldstatelabs.com/hub/<author>/<suffix>>
  once published. Suffix is chosen during planning.
- **Current stage**: `planning` (interruptible) — interview starts next.
- **Where source files land on disk**: `~/.config/choreo/workflows/<suffix>/`
- **Cloud publish** (mode=cloud only): auto-runs after the validator
  ...
  `/choreo:publish <target-dir>`.
- **Abort**: `/choreo:cancel` or `/choreo:interrupt` anytime.
```
...
- **Template being edited**: cloud://<name>
  → <choreo.worldstatelabs.com/hub/<name>>
- **Current stage**: `planning` (interruptible) — interview starts next.
- **Source dir (local checkout for this edit)**: `~/.config/choreo/workflows/<name>/`
- **Cloud publish** (mode=cloud only): auto-runs after the validator
...
- Both Create and Edit dispatch the SAME choreo; the only difference is `CREATE_WORKFLOW_CONTEXT`.
...
- Cloud publish is handled by the choreo's `publishing` stage (gated on `publish_intent` in `setup_context`). The skill itself does not wait for or call publish.
