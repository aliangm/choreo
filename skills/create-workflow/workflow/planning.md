...
- `mode`: `"create"` or `"edit"`
- `description`: the original natural-language description the user passed to `/choreo:create` (may be empty). **This is the only place the description shows up** — it is NOT in `state.md`'s `topic:` field.
...
- **Create mode**: derive a short, kebab-case suffix from the description (e.g. "Python library dev with docs and publish" → `python-lib`). Confirm with the user. Target directory: `~/.config/choreo/workflows/<suffix>/`. If the directory already exists, ask whether to overwrite or pick a different name.
...
- `modifies_worktree: false` — <one-line reason, e.g. "writes to ~/.config/choreo, never touches project dir">
...
2. Build the review pointer:
   - **`publish_intent == "cloud"`**: the live session URL.
     ...
     - `SERVER`: read from `~/.cache/choreo/cloud-registry/<SESSION_ID>.json` `.server` field. Falls back to `https://choreo.worldstatelabs.com` if the file or field is missing.
