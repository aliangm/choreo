# create-workflow (choreo)

Turns a natural-language description into a validated workflow definition via a plan → write → validate loop, then pushes to the hub when the caller asked for cloud mode. This is the default choreo that drives `/choreo:create`.
...
The caller (SKILL.md for `/choreo:create`) exports `CREATE_WORKFLOW_CONTEXT` as a JSON env var before dispatch. The `setup_context` run_file captures it at setup time so all stages can read it:
...
Invoked automatically by `/choreo:create`. Not intended to be launched directly via `/choreo:start`.
