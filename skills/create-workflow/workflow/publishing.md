...
**Purpose:** After the validator confirms the workflow files are correct, push the workflow to the hub if the user invoked `/choreo:create` with `--mode=cloud`. For `--mode=local` this stage is a no-op pass-through.
...
- Non-zero → publish failed. **Still write `result: done`** (the workflow files are valid locally — the failure is at push time, not fatal to the run). Include the full `OUTPUT` verbatim so the user can diagnose and retry with `/choreo:publish <target-dir>`.
...
- On failure (non-zero): "Publish failed — files are valid locally at `<target-dir>`. Retry with `/choreo:publish <target-dir>` after resolving the issue. Common causes: expired token (`/choreo:login` again), network, or name collision with another user."
````
