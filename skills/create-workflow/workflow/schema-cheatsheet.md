# Choreo Workflow Cheatsheet — Schema + Claude Code Runtime Constraints
...
| `modifies_worktree` | no (default true) | Set to `false` when the workflow writes **nothing** to the project dir. Example: `create-workflow` (writes to `~/.config/choreo/`), `publish-workflow` (pure HTTP calls). When `false`, the plugin skips worktree-diff capture and UI hides the diff panel. |
...
| **Subagent tool allowlist is fixed** | `choreo:workflow-subagent` ships with `Bash`, `Read`, `Write`, `Edit`, `Glob`, `Grep`, `WebFetch`, `WebSearch`, `NotebookEdit`. No `Agent`, no `AskUserQuestion`, no MCP-only tools, no custom slash-commands. | If a stage needs a tool outside this set (e.g. project MCP server, custom command), it MUST be an inline stage. |
