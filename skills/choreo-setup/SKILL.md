---
name: choreo-setup
description: "Bootstrap a dev workflow session (for /choreo:start): parse flags, derive a topic from the task description, call setup-workflow.sh, and hand off to the workflow loop skill. Does NOT drive the state machine — that's choreo:choreo's job."
---

# Workflow Setup

Single-purpose skill: **materialize a new `state.md`** for the current Claude Code session by parsing the user's flags, picking a kebab-case topic, and invoking `setup-workflow.sh`. Used by `/choreo:start`; not invoked directly by users.
...
| Read `state.md`, run stage loop, post artifacts, advance state machine | **`choreo:choreo` skill (not this one)** |

By the time this skill returns control, `state.md` exists at the session's run dir. The caller (`commands/start.md`) then invokes `choreo:choreo` to drive the loop.
...
**Exit 0 — success.** `setup-workflow.sh` already printed the run directory and the initial stage's I/O context. Tell the user **exactly one** line: `Workflow session initialised; stage loop will take over.` Then return control — do not invoke any other skill or script from here. `commands/start.md` Step 2 invokes `choreo:choreo` to start the loop.
...
- `/choreo:continue` — resume the existing workflow (treat this new request as the one to discard)
- `/choreo:cancel` — archive or hard-delete the existing run, then retry `/choreo:start` with the new task
...
- This skill **does not drive the stage loop**. It exits after `setup-workflow.sh` succeeds. The loop is `choreo:choreo`'s job.
