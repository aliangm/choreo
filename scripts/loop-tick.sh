...
# choreo. The main agent calls this once per loop iteration and
...
VIEW_URL=""
if is_cloud_session "$RUN_DIR_NAME" && [[ -n "${CHOREO_SERVER:-}" ]]; then
  VIEW_URL="${CHOREO_SERVER}/s/${RUN_DIR_NAME}"
fi
