#!/bin/bash
# Print the pre-flight banner for /choreo:create
# Usage: print-create-banner.sh <mode> <workflow_flag> <wf_type>
#
# First-time-user framing problem: /choreo:create runs a choreo
...
if [[ -n "$WORKFLOW_FLAG" ]]; then
  ...
  if [[ "$WF_TYPE" == "cloud" ]]; then
    ...
    echo "  On finish: local files updated (run /choreo:publish to share)"
  fi
else
  ...
  if [[ "$MODE" == "cloud" ]]; then
    ...
    if [[ "$_logged_in" = yes ]]; then
      ...
      echo "  Auth:      ❌ not signed in — /choreo:login required for cloud"
    fi
  else
    echo "  Will save: ~/.config/choreo/workflows/<suffix>/"
