...
# Only works when status is "interrupted" — use /choreo:start for a fresh start.
#
# Session-keyed model: each run lives under .choreo/<session_id>/.
...
if [[ -n "${DESIRED_SESSION:-}" ]] && ! is_cloud_session "$DESIRED_SESSION"; then
  ...
  cloud_register_session "$DESIRED_SESSION" "${CHOREO_SERVER}" "" "$scratch_path"
...
#   2. Otherwise, scan all .choreo/*/ for a single interrupted run
...
if [[ -n "${DESIRED_SESSION:-}" ]]; then
  ...
  # Cloud short-circuit: .choreo/ doesn't exist in cloud mode, so
  ...
  if [[ -n "$_cur_sid" ]] && is_cloud_session "$_cur_sid"; then
    ...
    if [[ "$rc" -ne 0 ]]; then
      ...
      if workflows=$(list_all_workflows); [[ -n "$workflows" ]]; then
        ...
        echo "   Start a new workflow with: /choreo:start <task>" >&2
      fi
      exit 1
    fi
  fi
...
if [[ "$STATUS" != "interrupted" ]]; then
  ...
  echo "     2. Run /choreo:interrupt (or /exit — the SessionEnd hook will" >&2
  echo "        auto-interrupt on graceful exit)." >&2
  echo "     3. Come back here and retry /choreo:continue." >&2
  exit 1
fi
...
if [[ "$IS_CLOUD" == "true" ]]; then
  ...
  if [[ -n "$_LOCAL_SID" ]] && [[ "$_LOCAL_SID" != "$RUN_DIR_NAME" ]] && ! is_cloud_session "$_LOCAL_SID"; then
    ...
    [[ -z "$_tgt_server" ]] && _tgt_server="${CHOREO_SERVER:-https://choreo.worldstatelabs.com}"
...
if is_terminal_status "$STATUS" 2>/dev/null; then
  case "$STATUS" in
    cancelled)
      ...
      echo "    Start a new workflow with /choreo:start if you want to retry." >&2
      ;;
    *)
...
if [[ "$STATUS" == "interrupted" ]]; then
  ...
  if [[ -z "$RESUME_STATUS" ]]; then
    ...
    echo "   Inspect $STATE_FILE and set resume_status manually, or start over with /choreo:start." >&2
    exit 1
  fi
...
if [[ "$IS_CLOUD" != "true" ]]; then
  ...
  if [[ -n "$NEW_SESSION" ]] && [[ "$NEW_SESSION" != "$OLD_SESSION" ]]; then
    NEW_DIR="${PROJECT_ROOT}/.choreo/${NEW_SESSION}"
    if [[ -e "$NEW_DIR" ]]; then
...
# /choreo:continue runs, the CC process that wrote those entries is
...
echo "   To interrupt again: /choreo:interrupt"
