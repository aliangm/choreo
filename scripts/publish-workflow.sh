...
#   /choreo:start --flow cloud://<name> <task>
...
Environment:
  CHOREO_SERVER      Hub server URL (default: baked-in plugin default).
...
if [[ -z "$DRY_RUN" ]] && ! cloud_is_logged_in; then
  ...
  echo "   Run /choreo:login first, then retry." >&2
  exit 1
fi
...
_PLUGIN_ROOT="$(cat ~/.config/choreo/plugin-root 2>/dev/null || true)"
[[ -d "${_PLUGIN_ROOT}/scripts" ]] || _PLUGIN_ROOT=~/.claude/plugins/choreo
[[ -d "${_PLUGIN_ROOT}/scripts" ]] || _PLUGIN_ROOT="$(ls -d ~/.claude/plugins/cache/*/choreo/*/ 2>/dev/null | head -1)"
if [[ -z "$DRY_RUN" ]]; then
...
if [[ -z "$NAME" ]]; then
  _author_raw="$(jq -r '.author // "anonymous"' "${HOME}/.config/choreo/auth.json" 2>/dev/null || echo "anonymous")"
...
URL="${CHOREO_SERVER}/api/workflows/${NAME}"
...
if [[ -z "$DRY_RUN" ]]; then
  ...
  if [[ $_pre_rc -ne 0 ]]; then
    cloud_explain_curl_exit "$_pre_rc" "$CHOREO_SERVER"
    rm -f "$_pre_tmp"
    exit 1
  fi
...
if [[ $put_rc -ne 0 ]]; then
  cloud_explain_curl_exit "$put_rc" "$CHOREO_SERVER"
  exit 1
fi
...
echo "   Template URL: ${CHOREO_SERVER}/hub/${NAME}"
echo ""
echo "   Pull with:"
echo "     /choreo:start --flow cloud://${NAME} <task>"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📍 RELAY THIS TO THE USER VERBATIM BEFORE CONTINUING:"
echo ""
echo "   Template published — URL:"
echo "   ${CHOREO_SERVER}/hub/${NAME}"
echo ""
