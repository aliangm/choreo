...
#
# Default:  move the run dir to .choreo/.archive/<ts>-<topic>-cancelled/
...
# ── Audit log ────────────────────────────────────────────────────
# Append one TSV row per cancel attempt to ~/.cache/choreo/cancel.log.
...
_AUDIT_SRV="?"
_AUDIT_LOG="${HOME}/.cache/choreo/cancel.log"
_emit_audit() {
  local rc=$?
  {
...
if ! resolve_state; then
  ...
  # fallback the user is stuck: /choreo:start refuses ("server has
  # active workflow"), /choreo:cancel refuses ("no local state"). We
  ...
  #       leaked registry so /choreo:start unblocks. Idempotent — if
...
if [[ -n "$LEDGER_AGENT_IDS" ]]; then
  echo "CHOREO_STOP_AGENT_IDS: $LEDGER_AGENT_IDS"
fi
...
# Default: archive to .choreo/.archive/<ts>-<topic>-cancelled/
