...
#
# Hits POST $CHOREO_SERVER/api/auth/device/code to get a short
...
# token to ~/.config/choreo/auth.json (mode 0600) so every subsequent
...
SERVER="${CHOREO_SERVER:-https://choreo.worldstatelabs.com}"
AUTH_DIR="${HOME}/.config/choreo"
...
if [[ "${1:-}" == "--demo" ]]; then
  if [[ -f "$AUTH_FILE" ]]; then
    ...
    if [[ -n "$existing_user" ]]; then
      ...
      echo "Run /choreo:logout first to switch to the demo account."
      exit 0
    fi
  fi
...
if [[ -f "$AUTH_FILE" ]]; then
  ...
  if [[ -n "$existing_user" ]]; then
    echo "Already signed in as: $existing_user"
    echo "Run /choreo:logout first if you want to switch accounts."
    exit 0
  fi
fi
...
while :; do
  ...
  case "$err" in
    ...
    access_denied)
      ...
      ;;
    expired_token)
      echo "❌ Code expired. Run /choreo:login to try again." >&2
      exit 1
      ;;
    *)
