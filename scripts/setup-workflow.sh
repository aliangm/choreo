...
#
# Two modes:
#   local (default)  workflow lives on disk under <project>/.choreo/
...
#                    nothing under .choreo/. A transient shadow at
#                    ~/.cache/choreo/sessions/<session_id>/ holds
...
#
# --flow accepts:
#   (omitted)          default:  ${PLUGIN_ROOT}/skills/choreo/workflow/
#   cloud://author/name cloud:   named template on $CHOREO_SERVER
...
#   • export CHOREO_DEFAULT_MODE=local in their shell env
MODE="${CHOREO_DEFAULT_MODE:-cloud}"
...
if [[ -z "$SESSION_ID" ]]; then
  ...
  echo "   the cache. Ensure the choreo plugin is properly installed" >&2
...
#
# Check order:
#   1. Cloud shadow  (~/.cache/choreo/sessions/<sid>/state.md)
#   2. Local run dir (<project>/.choreo/<sid>/state.md)
#   3. Cloud registry (~/.cache/choreo/cloud-registry/<sid>.json, shadow lost)
...
if [[ -z "$FORCE" ]]; then
  ...
  # 1. Cloud shadow
  _cs="${HOME}/.cache/choreo/sessions/${SESSION_ID}/state.md"
  if [[ -f "$_cs" ]]; then
  ...
  _ld="${PROJECT_ROOT}/.choreo/${SESSION_ID}/state.md"
  ...
  _cr="${HOME}/.cache/choreo/cloud-registry/${SESSION_ID}.json"
      ...
      case "$_srv_status" in complete|escalated|cancelled|"") ;;
        *)
          if [[ "$_srv_active" == "true" ]]; then
            ...
            _active_loc="${CHOREO_SERVER}/s/${SESSION_ID}"
          fi
          ;;
      esac
    elif [[ -n "${CHOREO_SERVER:-}" ]]; then
  ...
  if [[ -n "$_active_status" ]]; then
    ...
    echo "" >&2
    echo "   /choreo:interrupt to pause, /choreo:continue to resume," >&2
    echo "   /choreo:cancel to stop the existing workflow first." >&2
    exit 2
  fi
...
if [[ "$MODE" == "cloud" ]]; then
  ...
  SCRATCH_DIR="${HOME}/.cache/choreo/sessions/${SESSION_ID}"
  WORKFLOW_CACHE="${SCRATCH_DIR}/.workflow-cache"

  # --force: mirror what /choreo:cancel does for cloud sessions so
  ...
  # which writes to ~/.config/choreo/ not the project).
  ...
  http_code=$(curl -sS -o "$tmp_body" -w "%{http_code}" \
      -X POST "${CHOREO_SERVER}/api/sessions/${SESSION_ID}/setup" \
      -H "$(_cloud_auth_header)" \
  ...
  if [[ "$http_code" == "409" ]]; then
    ...
    echo "    Use /choreo:cancel to stop the existing run first." >&2
    rm -rf "$SCRATCH_DIR"
    exit 2
  fi
...
EOF
  ...
  cloud_register_session "$SESSION_ID" "$CHOREO_SERVER" "$WORKFLOW_URL"
  ...
  # Force config_show_stage_context to print shadow paths, not ${project}/.choreo/.
  DW_RUN_BASE="${HOME}/.cache/choreo/sessions"
  export DW_RUN_BASE
  ...
  echo "   Server:     $CHOREO_SERVER"
  echo "   UI:         ${CHOREO_SERVER}/s/${SESSION_ID}"
  echo "   To pause:  /choreo:interrupt"
  echo "   To cancel: /choreo:cancel"
  echo ""
  echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
  echo "📍 RELAY THIS TO THE USER VERBATIM BEFORE CONTINUING:"
  echo ""
  echo "   Live session URL:"
  echo "   ${CHOREO_SERVER}/s/${SESSION_ID}"
  echo ""
  ...
  # ~/.config/choreo/auth.json, they're running as an anonymous capability
...
SESSION_RUN_DIR="${PROJECT_ROOT}/.choreo/${SESSION_ID}"
...
rm -f "${PROJECT_ROOT}/.choreo/state.md"
