...
if ! resolve_state; then
  ...
  if workflows=$(list_all_workflows); [[ -n "$workflows" ]]; then
    ...
    echo "   No workflows found. Run /choreo:start to start one." >&2
  fi
  exit 1
fi
...
if ! is_terminal_status "$NEW_STATUS" && [[ "$NEW_EPOCH" -ge "$MAX_EPOCH" ]]; then
  if config_is_terminal "escalated"; then
    echo "⚠️  [choreo] epoch $NEW_EPOCH reached max-epoch $MAX_EPOCH — escalating (was heading to '$NEW_STATUS')" >&2
    NEW_STATUS="escalated"
  else
    echo "⚠️  [choreo] epoch $NEW_EPOCH reached max-epoch $MAX_EPOCH but 'escalated' is not declared under .terminal_stages — proceeding to '$NEW_STATUS' without auto-escalation" >&2
  fi
fi
...
if is_cloud_session "$RUN_DIR_NAME"; then
  ...
  if is_terminal_status "$NEW_STATUS"; then
    # Terminal artifact: per `skills/choreo/SKILL.md` (200ff) the
...
echo "[choreo] Topic: $TOPIC | Status: $NEW_STATUS | epoch: $NEW_EPOCH"
