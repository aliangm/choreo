#!/bin/bash
# Shared utilities for choreo scripts.
#
# Groups:
#   1. .choreo/ discovery (find_dw_root)
...
# Default ships at skills/choreo/workflow/.
DEFAULT_WORKFLOW_DIR="${PLUGIN_ROOT}/skills/choreo/workflow"
...
# Echo the absolute path to the nearest .choreo/ dir (upward walk from CWD).
...
# script silently — exactly what bit cross-machine `/choreo:continue`
...
# ──────────────────────────────────────────────────────────────
# bootstrap_completed_at — "the choreo skill driver has engaged
...
# "no driver has engaged yet → block to force `Skill("choreo:choreo")`
...
# ──────────────────────────────────────────────────────────────
# Archive helper — move a run dir to .choreo/.archive/
# ──────────────────────────────────────────────────────────────
#
...
# lives in one place.
#
# Archive path: <.choreo>/.archive/<YYYYMMDD-HHMMSS>-<topic>[-<suffix>]/
# Hidden dot-dir so resolve_state's "$dw"/*/state.md glob skips it.
#
# Args:
#   $1 = run dir (absolute), typically .choreo/<session_id>
...
_DW_SESSION_CACHE_DIR="${HOME}/.cache/choreo/session-cache"
...
# ──────────────────────────────────────────────────────────────
#
# Layout: <project>/.choreo/<session_id>/state.md plus per-stage
...
resolve_state() {
  ...
  # project's worktree has no .choreo/ in cloud mode, so walking up
  ...
  # Session-keyed layout: .choreo/<session_id>/state.md
  ...
  # session subdir under .choreo/, just use it. This removes the
  ...
  # .choreo/ happens to have exactly one entry.
  if [[ -z "$session" ]]; then
    local _candidates=()
    local _c
  ...
  # Legacy: flat .choreo/state.md (pre-v1.11) — single-workflow fallback
...
# .choreo/. Used by continue-workflow.sh for cross-session takeover.
...
# List all workflows (state.md files) under .choreo/, with their status.
...
config_validate() {
  ...
  while read -r stage; do
    ...
    # choreo:workflow-subagent hardcoded in agent-guard.sh and
...
# project (e.g. create-workflow writes to ~/.config/choreo/).
#
...
#   3. Fallback: <project>/.choreo/<run_dir_name>/<stage>-report.md —
...
config_artifact_path() {
  ...
  echo "${project_root}/.choreo/${run_dir_name}/${stage}-report.md"
}
...
config_run_file_path() {
  ...
  echo "${project_root}/.choreo/${run_dir_name}/${name}"
}
...
# ~/.cache/choreo/sessions/<session_id>/ so Claude's Read/Write
# tools still have real file paths to operate on. Every write is mirrored
# to the server via curl. The project worktree gets no .choreo/ dir.
#
# Registry: ~/.cache/choreo/cloud-registry/<session_id>.json records
...
CLOUD_REGISTRY_DIR="${HOME}/.cache/choreo/cloud-registry"
CLOUD_SCRATCH_BASE="${HOME}/.cache/choreo/sessions"

# Default cloud server for this plugin build. Hard-coded so users only need
# to export CHOREO_API_TOKEN; the server URL is baked in. Override by
# exporting CHOREO_SERVER=... (useful for pointing at a local dev
# webapp, a staging deployment, or a fork).
: "${CHOREO_SERVER:=https://choreo.worldstatelabs.com}"
export CHOREO_SERVER
...
cloud_require_env() {
  ...
  # CHOREO_SERVER). This function stays in place so a future
  # multi-user auth layer can plug back in without touching callers.
  if [[ -z "${CHOREO_SERVER:-}" ]]; then
    echo "❌ CHOREO_SERVER unexpectedly empty" >&2
    return 1
  fi
  return 0
}
...
#
#   - Authenticated: ~/.config/choreo/auth.json exists with a `token`
...
#     any rows it creates.
#
#   - Anonymous: no auth file. We emit a benign X-Choreo marker
...
#
# To log in:  /choreo:login
# To log out: /choreo:logout
# Returns 0 if the user has a non-empty bearer token at
# ~/.config/choreo/auth.json (written by login-workflow.sh), else 1.
...
cloud_is_logged_in() {
  local auth_file="${HOME}/.config/choreo/auth.json"
...
cloud_explain_curl_exit() {
  local code="$1" server="${2:-the choreo server}"
  case "$code" in
    0)   return 0 ;;
...
#   cloud://author/name  named template on $CHOREO_SERVER
#   /abs/path         local absolute path (copied verbatim)
#   bare name         resolved against PLUGIN_ROOT/skills/choreo/
#
...
cloud_fetch_workflow_from_name() {
  ...
  mkdir -p "$dest"
  local base="${CHOREO_SERVER}/api/workflows/${name}"
  local bundle
  local fetch_rc=0
  ...
  if [[ $fetch_rc -ne 0 ]]; then
    ...
    if [[ $fetch_rc -eq 22 ]]; then
      ...
      echo "   Check the name, or run /choreo:login if it's private." >&2
    else
      cloud_explain_curl_exit "$fetch_rc" "$CHOREO_SERVER"
    fi
    return 1
  fi
...
cloud_post_setup() {
  ...
  _cloud_post_json "${CHOREO_SERVER}/api/sessions/${sid}/setup" "$payload"
}
...
cloud_post_state() {
  ...
  if ! _cloud_post_json "${CHOREO_SERVER}/api/sessions/${sid}/state" "$payload"; then
...
cloud_post_artifact() {
  ...
  if ! _cloud_curl_retry POST \
        "${CHOREO_SERVER}/api/sessions/${sid}/artifacts/${stage}" \
...
cloud_delete_artifact() {
  ...
  if ! _cloud_curl_retry DELETE \
        "${CHOREO_SERVER}/api/sessions/${sid}/artifacts/${stage}"; then
...
cloud_post_archive() {
  local sid="$1"
  cloud_require_env || return 1
  if ! _cloud_curl_retry POST "${CHOREO_SERVER}/api/sessions/${sid}/archive"; then
...
cloud_post_cancel() {
  local sid="$1"
  cloud_require_env || return 1
  if ! _cloud_curl_retry POST "${CHOREO_SERVER}/api/sessions/${sid}/cancel"; then
...
cloud_session_status_class() {
  ...
  http="$(curl -sS -o "$body" -w "%{http_code}" --max-time 5 \
    -H "$(_cloud_auth_header)" \
    "${CHOREO_SERVER}/api/sessions/${sid}" 2>/dev/null)" || http="000"
  case "$http" in
...
cloud_delete_session() {
  local sid="$1"
  cloud_require_env || return 1
  if ! _cloud_curl_retry DELETE "${CHOREO_SERVER}/api/sessions/${sid}"; then
...
# Used by continue-workflow.sh when the user runs `/choreo:continue
...
cloud_pull_shadow() {
  ...
  _pull_http="$(curl -sS -o "$_pull_tmp" -w "%{http_code}" \
      -H "$(_cloud_auth_header)" \
      "${CHOREO_SERVER}/api/sessions/${sid}" 2>/dev/null)" || _pull_http="000"
  ...
  while read -r fname; do
    ...
    curl -sS -fL -H "$(_cloud_auth_header)" \
      ...
      "${CHOREO_SERVER}/api/sessions/${sid}/files/${fname}" 2>/dev/null || {
...
EOF
  ...
  diff_resp="$(curl -sS -fL -H "$(_cloud_auth_header)" \
               "${CHOREO_SERVER}/api/sessions/${sid}/diff" 2>/dev/null || echo "{}")"
...
cloud_post_diff() {
  ...
  # writes to ~/.config/choreo/) opt out of the diff pipeline.
  ...
  local -a DIFF_EXCLUDES=(
    ':(top,exclude).choreo' ':(top,exclude).choreo/**'
    ':(top,exclude,glob)**/.choreo/**'
  ...
  if ! _cloud_post_json "${CHOREO_SERVER}/api/sessions/${sid}/diff" "$payload"; then
...
cloud_post_activity() {
  ...
  local _alog_file="${HOME}/.cache/choreo/activity-hook.log"
...
ensure_git_baseline() {
  ...
  if ! git -C "$proot" rev-parse HEAD >/dev/null 2>&1; then
    ...
    git -C "$proot" \
      -c user.name='choreo' \
      -c user.email='choreo@local' \
      commit --allow-empty -q -m "choreo: initial baseline (topic=${topic})"
    if [[ -n "$has_files" ]]; then
...
cloud_reconcile_state() {
  ...
  snapshot="$(curl -sS -fL --max-time 3 \
              -H "$(_cloud_auth_header)" \
              "${CHOREO_SERVER}/api/sessions/${sid}" 2>/dev/null)" || return 0
