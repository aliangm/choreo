...
WORKFLOW_DIR="${PLUGIN_ROOT}/skills/choreo/workflow"

echo "T6 — stage files: no hardcoded run-dir path placeholders"

# No stage file should contain <project>/.choreo/<session_id> style paths
for f in "${WORKFLOW_DIR}"/*.md; do
  [[ "$(basename "$f")" == "run_files_catalog.md" ]] && continue
  name="$(basename "$f")"

  ! grep -q '\.choreo/<session_id>' "$f"
  check "${name}: no .choreo/<session_id> placeholder" $?

  ! grep -q '<project>/\.choreo' "$f"
  check "${name}: no <project>/.choreo placeholder" $?
done
...
! grep -q '<project>/\.choreo' "${WORKFLOW_DIR}/reviewing.md"
