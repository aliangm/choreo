...
# We override HOME so the script reads from our fake ~/.config/choreo/auth.json
# rather than the real user's. CHOREO_SERVER is set to an unreachable
...
mkdir -p "$FAKE_HOME"
out1="$(HOME="$FAKE_HOME" CHOREO_SERVER="$DEAD_SERVER" "$WHOAMI" 2>&1)"
...
mkdir -p "$FAKE_HOME/.config/choreo"
cat > "$FAKE_HOME/.config/choreo/auth.json" <<'EOF'
{
  "label": "my-laptop",
  "author": "alice",
...
EOF
out3="$(HOME="$FAKE_HOME" CHOREO_SERVER="$DEAD_SERVER" "$WHOAMI" 2>&1)"
