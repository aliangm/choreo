#!/bin/bash
# E2E-1 — choreo SKILL: real Claude execution
#
# Invokes the choreo skill via `claude --print` and asserts on the
...
echo "E2E-1 — choreo SKILL (real Claude)"
...
find_run_dir() {
  local project="$1"
  find "$project/.choreo" -maxdepth 1 -mindepth 1 -type d \
...
find_state() {
  local project="$1"
  find "$project/.choreo" -name "state.md" \
...
run_claude "$P1" \
    "/choreo:start --mode=local --flow=${SMOKE_WF} smoke-check-e2e" \
    OUTPUT1 RC1
...
check "E2E-1-1: run dir created under project .choreo/" "$rc_rd1"
...
# real Claude via /choreo:continue and verify it drives the stage to done.
...
STATE3="$P3/.choreo/$SID3/state.md"
...
if [[ -f "$STATE3" ]]; then
  ...
  if [[ "$int_status" == "interrupted" ]]; then
    ...
    run_claude "$P3" "/choreo:continue" OUT3_CONT RC3_CONT
    check "E2E-1-3: /choreo:continue (real Claude) exits 0" "$RC3_CONT"
    ...
    check "E2E-1-3: /choreo:continue (real Claude) exits 0" 1
...
STATE4="$P4/.choreo/$SID4/state.md"
