...
WF_DIR1="$HOME/.config/choreo/workflows/lint-fix-${SUFFIX}"
[[ -d "$WF_DIR1" ]]
check "E2E-2-1: workflow directory created at ~/.config/choreo/workflows/" $?
...
FAKE_HOME="$TMP/home"
mkdir -p "$FAKE_HOME/.cache/choreo/session-cache"
P4="$TMP/project4"
mkdir -p "$P4"
git -C "$P4" init -q
...
echo "e2e-sess-${SUFFIX}" > "$FAKE_HOME/.cache/choreo/session-cache/cwd-$key4"
...
STATE4="$P4/.choreo/e2e-sess-${SUFFIX}/state.md"
[[ -f "$STATE4" ]]
...
PROMPT5="$(cat <<EOF
/choreo:create --mode=local --flow=${WF_DIR1}
