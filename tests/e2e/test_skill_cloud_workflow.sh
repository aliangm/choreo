#!/bin/bash
# C2E-2 — choreo SKILL: cloud mode E2E (real Claude + real server)
#
# Invokes the choreo skill in cloud mode via `claude --print` and
...
#   ~/.cache/choreo/sessions/<UUID>/state.md. On terminal status,
#   both update-status.sh and stop-hook.sh wipe the shadow + unregister
#   from ~/.cache/choreo/cloud-registry/. The project worktree never gets
#   a .choreo/ directory in cloud mode.
#
# Requirements: claude CLI in PATH, API access, network access to
#   CHOREO_SERVER (defaults to https://choreo.worldstatelabs.com).
...
echo "C2E-2 — choreo SKILL: cloud mode (real Claude + real server)"
...
CHOREO_SERVER="${CHOREO_SERVER:-https://choreo.worldstatelabs.com}"
...
cleanup() {
  for sid in "${CREATED_SESSIONS[@]}"; do
    ...
    curl -sS --max-time 5 \
      -X DELETE "${CHOREO_SERVER}/api/sessions/${sid}" \
...
# The session-start hook writes ~/.cache/choreo/session-cache/cwd-<sha1> each
...
read_project_session() {
  ...
  local cache_file="${HOME}/.cache/choreo/session-cache/cwd-${key}"
...
write_session_cache() {
  ...
  local key
  key="$(cd "$project" && printf '%s' "$(pwd)" | shasum -a 1 | cut -c1-16)"
  mkdir -p "${HOME}/.cache/choreo/session-cache"
  echo "$session_id" > "${HOME}/.cache/choreo/session-cache/cwd-${key}"
}
...
seed_cloud_state() {
  ...
  local scratch="${HOME}/.cache/choreo/sessions/${session_id}"
  ...
  # Register locally.
  cloud_register_session "$session_id" "$CHOREO_SERVER" ""
...
server_session_status() {
  ...
  resp="$(curl -sS -fL --max-time 10 \
    -H "$(_cloud_auth_header)" \
    "${CHOREO_SERVER}/api/sessions/${sid}" 2>/dev/null)" || { echo ""; return; }
...
run_claude "$P1" \
    "/choreo:start --mode=cloud --flow=${SMOKE_WF} smoke-cloud-e2e" \
    OUTPUT1 RC1
...
if [[ -n "$SID1" ]]; then
  ...
  SHADOW1="${HOME}/.cache/choreo/sessions/${SID1}"
  ...
  REG1="${HOME}/.cache/choreo/cloud-registry/${SID1}.json"
  ...
  # No .choreo/ in the project (cloud doesn't pollute the worktree).
  rc_nolocal1=0; [[ ! -d "$P1/.choreo" ]] || rc_nolocal1=1
  check "C2E-2-1: no .choreo/ in project worktree" "$rc_nolocal1"
else
  ...
  check "C2E-2-1: no .choreo/ in project worktree" 1
fi
...
SHADOW3="${HOME}/.cache/choreo/sessions/${SID3}"
...
run_claude "$P3" "/choreo:continue --session ${SID3}" OUT3_CONT RC3_CONT
check "C2E-2-3: /choreo:continue --session (real Claude) exits 0" "$RC3_CONT"
...
if [[ -d "$SHADOW3" ]] && [[ -f "${SHADOW3}/analyze-report.md" ]]; then
  ...
  srv_resp3="$(curl -sS -fL --max-time 10 \
    -H "$(_cloud_auth_header)" \
    "${CHOREO_SERVER}/api/sessions/${SID3}" 2>/dev/null || echo "{}")"
...
REG3="${HOME}/.cache/choreo/cloud-registry/${SID3}.json"
...
# /choreo:continue --session UUID from the same project dir.
...
SHADOW4="${HOME}/.cache/choreo/sessions/${SID4}"
...
rm -rf "${HOME}/.cache/choreo/sessions/${SID4}"
rm -f "${HOME}/.cache/choreo/cloud-registry/${SID4}.json"

rc_noshadow4=0; [[ ! -d "${HOME}/.cache/choreo/sessions/${SID4}" ]] || rc_noshadow4=1
check "C2E-2-4: shadow wiped (simulating machine switch)" "$rc_noshadow4"

rc_noreg4=0; [[ ! -f "${HOME}/.cache/choreo/cloud-registry/${SID4}.json" ]] || rc_noreg4=1
...
run_claude "$P4" "/choreo:continue --session ${SID4}" OUT4_CONT RC4_CONT
check "C2E-2-4: /choreo:continue --session UUID exits 0" "$RC4_CONT"
...
srv_resp4="$(curl -sS -fL --max-time 10 \
  -H "$(_cloud_auth_header)" \
  "${CHOREO_SERVER}/api/sessions/${SID4}" 2>/dev/null || echo "{}")"
...
rc_shadow4f=0; [[ ! -d "${HOME}/.cache/choreo/sessions/${SID4}" ]] || rc_shadow4f=1
