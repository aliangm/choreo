...
mkdir -p "$PROJ/.choreo/$SID"
cat > "$PROJ/.choreo/$SID/state.md" <<EOF
---
topic: cap-test
status: planning
epoch: 2
...
new_status="$(grep '^status:' "$PROJ/.choreo/$SID/state.md" | awk '{print $2}')"
...
s5="$(grep '^status:' "$PROJ/.choreo/$SID/state.md" | awk '{print $2}')"
[[ "$s5" == "cancelled" ]]
...
SID6="fake-session-6"
mkdir -p "$PROJ6/.choreo/$SID6"
cat > "$PROJ6/.choreo/$SID6/state.md" <<EOF
---
...
cat > "$PROJ6/.choreo/$SID6/planning-report.md" <<EOF
---
epoch: 2
result: done
---
...
s6="$(grep '^status:' "$PROJ6/.choreo/$SID6/state.md" | awk '{print $2}')"
[[ "$s6" == "reviewing" ]]
