...
mkdir -p "$FAKE_HOME2/.config/choreo"
cat > "$FAKE_HOME2/.config/choreo/auth.json" <<'EOF'
{
...
[[ ! -f "$FAKE_HOME2/.config/choreo/auth.json" ]]
...
mkdir -p "$FAKE_HOME3/.config/choreo"
cat > "$FAKE_HOME3/.config/choreo/auth.json" <<'EOF'
{
...
mkdir -p "$FAKE_HOME4/.config/choreo"
echo '{"label": "anon-device"}' > "$FAKE_HOME4/.config/choreo/auth.json"
...
mkdir -p "$FAKE_HOME5/.config/choreo"
echo '{"user_id": "demo:1"}' > "$FAKE_HOME5/.config/choreo/auth.json"
