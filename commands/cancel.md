...
**Default behaviour**: archives the run dir to `.choreo/.archive/<YYYYMMDD-HHMMSS>-<topic>-cancelled/` so all stage reports and the baseline survive as an audit trail. The dir name's `-cancelled` suffix distinguishes cancelled runs from natural replacements.
...
```
CHOREO_STOP_AGENT_IDS: <id1> <id2> ...
```

## Step 2 — Stop each in-flight subagent

If Step 1 printed a `CHOREO_STOP_AGENT_IDS:` line, call the `TaskStop` tool **once per id** to terminate the orphan subagents. If it didn't print that line, skip this step — there were no in-flight subagents.
