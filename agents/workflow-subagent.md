...
description: |
  Generic stage executor for the choreo plugin. Launched by the
...
# skills/choreo/SKILL.md (subagent must not invoke external skills).
tools: Bash, Read, Write, Edit, Glob, Grep, WebFetch, WebSearch, NotebookEdit
---

You are a choreo stage executor. Your job is to run **one stage** of a workflow and write its output artifact.
