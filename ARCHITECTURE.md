...
```
commands/
  start.md               ← /choreo:start — kick off a run
...
```
~/.config/choreo/          ← persistent (do not clear)
...
~/.cache/choreo/           ← ephemeral (safe to clear; rebuilt on next use)
...
Per-project state (`<project>/.choreo/<session_id>/`, local mode) is unaffected — still tracked per project.
...
| `modifies_worktree` | Optional, boolean. Default `true`. When `false` the plugin skips worktree-diff capture (no `baseline-tree` snapshot, no `cloud_post_diff` POST) and the UI hides the Working-tree diff panel in favour of a placeholder. Set `false` for workflows whose output lives outside the project worktree — e.g. `create-workflow` writes to `~/.config/choreo/workflows/`, `publish-workflow` only makes HTTP calls. Leave `true` (or omit) for coding workflows whose subagents edit project source files |
...
Per-stage `subagent_type` is **NOT supported** — all subagent stages run under the single generic `choreo:workflow-subagent`. The validator rejects it.
...
| Status is `interrupted` | Allows exit; keeps `state.md` for `/choreo:continue` |
...
```
USER  ► /choreo:start Build a note-taking app
...
`/choreo:continue --session <id>` on a machine that has never seen this cloud session:
